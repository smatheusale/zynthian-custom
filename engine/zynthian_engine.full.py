...
class zynthian_engine(zynthian_basic_engine):
    ...
    def __init__(self, state_manager=None):
        ...
        # Engines that load patches via set_preset (MIDI dispatch or file load)
        # must opt in so snapshot restore re-fires set_preset after JACK ports
        # are wired. See zynthian_state_manager.load_snapshot.
        self.needs_set_preset_after_restore = False
    ...
    def set_preset(self, processor, preset, preload=False):
        if isinstance(preset[1], int):
            self.state_manager.zynmidi.set_midi_prg(
                processor.get_midi_chan(), preset[1])
        else:
            self.state_manager.zynmidi.set_midi_preset(
                processor.get_midi_chan(), preset[1][0], preset[1][1], preset[1][2])
        return True
    ...
    def load_preset_favs(self):
        if self.nickname:
            ...
            self.preset_favs_fpath = self.my_data_dir + \
                "/preset-favorites/" + fname + ".json"
